...
export class CartService {
  ...
  getCart(id: string) {
    ...
    // pipe returns observable, allows to work with it | while subscribe returns subscription, not an observable
  }
  ...
  async removeItemFromCart(productId: number, quantity = 1) {
    ...
    if (index !== -1) {
      // check if we are passing quantity higher than we have in a cart
      if (cart.items[index].quantity > quantity) {
        cart.items[index].quantity -= quantity;
      } else {
        //remove item from cart if passed quantity is equal or grater then quantity of existing items in cart
        cart.items.splice(index, 1);
      }
      if (cart.items.length === 0) {
        this.deleteCart();
      } else {
        await firstValueFrom(this.setCart(cart)); // update cart on redis server
      }
    }
  }

  deleteCart() {
    // delete on redis server and delete cart id from local storage
  ...
  private isProduct(item: CartItem | Product): item is Product {
    // product has id prop but cart item has not
