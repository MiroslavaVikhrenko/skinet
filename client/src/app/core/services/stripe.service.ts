...
export class StripeService {
  ...
  private accountService = inject(AccountService); // populate default address
  ...
  async initializeElements() { // user getting to the checkout
    // ensure we use the same instance
    if (!this.elements) {
      const stripe = await this.getStripeInstance();
      if (stripe) {
        // create or update payment intent if stripe is not null
  ...
  async createConfirmationToken() {
    const stripe = await this.getStripeInstance();
    const elements = await this.initializeElements();
    // Before confirming payment, call elements.submit() 
    // to validate the state of the Payment Element and collect any data required for wallets
  ...
  createOrUpdatePaymentIntent() {
    ...
    return this.http.post<Cart>(this.baseUrl + 'payments/' + cart.id, {}).pipe(
      map(cart => {
        // also update redis, not only locally
