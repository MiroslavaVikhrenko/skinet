...
public class AdminController(IUnitOfWork unit, IPaymentService paymentService) : BaseApiController
{
    ...
    public async Task<ActionResult<OrderDto>> GetOrderById(int id)
    {
        ...
        if (order == null) return BadRequest("No order with that id"); // won't redirect admin to another page
    ...
    public async Task<ActionResult<OrderDto>> RefundOrder(int id)
    {
        ...
        // cannot refund orders with pending states as payment was not received
        // such orders in dev, but not prod as orders created after payment received
